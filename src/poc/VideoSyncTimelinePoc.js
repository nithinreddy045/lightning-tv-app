...
streamStopped: false,
  ...
  try {
    ...
    const commonTimelineStart =
      Number(session.commonTimelineStart)

    const serverNow =
      Number(session.serverNow)

    if (
      !Number.isFinite(commonTimelineStart) ||
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      const currentCommonPosition =
        (serverNow - commonTimelineStart) / 1000
...
getPlaylistPosition() {
  ...
  return {
    ...
    resumeIfDue() {
      if (this.video && this.video.paused && !this.buffering && !this.userPaused) {
        this.video.play().catch((error) => {
...
const videoUrl = firstVideo.url
      ...
      if (Hls.isSupported()) {
        ...
        hls.on(Hls.Events.ERROR, (event, data) => {
          console.error('POC: HLS ERROR:', JSON.stringify(data))
